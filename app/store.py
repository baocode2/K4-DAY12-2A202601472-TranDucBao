...
class ChatStore:
    ...
    def ping(self) -> bool:
        ...
        try:
            ...
            # /readyz gọi hàm này — để exception thoát ra là biến readiness
            # probe thành lỗi 500 thay vì một câu trả lời "chưa sẵn sàng".
            return False
    ...
    def add_turn(self, client_id: str, role: str, content: str) -> None:
        ...
        # giữ HISTORY_MAX_MESSAGES message MỚI NHẤT (đếm từ cuối list)
    ...
    def history(self, client_id: str) -> list[dict]:
        ...
        """
        raw = self.client.lrange(self._key(client_id), 0, -1) or []
